...
$.global = {
  ...
  getImgPath: function () {
    return $.global.imgPath
  },
  getDBType: function () {
    return $.global.dbType
  },
  ...
  setDBType: function (v) {
    $.global.dbType = v
    return $.global
  },
  setIfCross: function (v) {
    $.global.ifCross = v
    return $.global
  }
}
...
setTimeout(function () {
  ...
  if (!window.MConfirm) {
    window.MConfirm = new $.UI.PopTips({
      type: 'confirm',
      y: 120,
      icon: 'icon-glyph-question-sign',
      ifShow: false,
      ifMask: true
    })
  }
  if (!window.MTips) {
    window.MTips = new $.UI.PopTips({
      type: 'tips',
      ifShow: false
    })
  }
  if (!window.MiniTips) {
    window.MiniTips = new $.UI.MiniTips({
      ifShow: false,
      dir: 'top',
      css: 'padding:3px 5px;border:1px solid #FFC97C;background-color:#FFFEC9;z-index:25;max-width:320px;'
    })
  }
  if (!window.ValidateTips) {
    window.ValidateTips = new $.UI.MiniTips({
      ifShow: false,
      css: 'padding:3px 5px;border:1px solid #FFC97C;color:#E52C01;background-color:#FFFEC9;z-index:2500;'
    })
  }
  if (!window.trace) {
    window.trace = function (v) {
      if (isIE) {
        alert(v)
      } else {
        console.log(v)
  ...
  $D.evt('click', function (e) {
    ...
    var e = $.e.fix(e),
      _e = e.t,
      _sMTips = _e.attr('MTips') || ''
    if (+_sMTips == 1) {
      _sMTips = _e.h() || _e.value
    }
    if (_sMTips) {
  ...
  $(window).evt('resize', function () {
    $.UI.DestroyPopElm('window--resize')
    if ($.global.arrowTips) {
...
$.initArrowTips = function (obj, css) {
  ...
  $.global.arrowTips = new $.UI.PopDialog({
    p: $DB,
    ePop: obj.Owner,
    ifClose: true,
    arrowBBC: '#FFF',
...
$.UI.Select = function (j) {
  ...
  me.setEnabled = function (v) {
    ...
    if (v) {
      eImg.dc('select-disabled').ac('select-normal')
    } else {
      eImg.dc('select-normal').ac('select-disabled')
    }
  ...
  me.setChecked = function (v, ifExec) {
    ...
    if (v) {
      eImg.dc('select-normal').ac('select-checked')
    } else {
      eImg.dc('select-checked').ac('select-normal')
    }
...
$.UI.ColorSelector = function (j) {
  ...
  function layout () {
    ...
    satP = owner.fc()
    satValImg = satP.fc()
    crossImg = satValImg.ns()
    allP = satP.ns()
    allColorImg = allP.fc()
    allSelectorImg = allColorImg.ns()
    preP = allP.ns().fc()
    inputP = preP.ns()
    trackDrag(satP, satValDragged)
    trackDrag(allP, allDragged)
    me.setValue(args.value)
  }
...
$.UI.RoleSelector = function (j) {
  ...
  function onAllCheck (obj) {
    ...
    if (+obj.Value) {
      ...
      for (var i = 0, _len = roles.length; i < _len; i++) {
        var _r = roles[i]
        _r.setChecked(_ifChecked, false)
        if (_ifChecked) {
          ids.push(_r.getValue())
          rids.push(_r.getText())
        }
      }
    }
  }
  me.setRoles = function (roleids) {
    ...
    ids = []
    rids = []
    for (var i = 0, _len = roleids.length; i < _len; i++) {
      if (roleids[i]) {
        _ids[roleids[i]] = true
      }
    }
    ...
    if (_count && _count == _rLen) {
      _selAll.setChecked(true, false)
    } else {
      _selAll.setChecked(false, false)
    }
  }
...
$.UI.SingleUserSelector = function (j) {
  ...
  function setDefault (j) {
    args = $.Util.initArgs(j, args)
    for (var i = 0, _lLen = _letAry.length; i < _lLen; i++) {
      var _l = _letAry[i]
      _LBtnAry.push({
        name: _l,
        text: _l,
  ...
  function layout () {
    owner = new $.UI.Tab({
      p: args.p,
      items: _iAry,
      ifFixedBodyHeight: false
    })
    var _eLet = owner.items['letter'].Body,
      _eDept = owner.items['dept'].Body
    var _eLBtn = _eLet.fc(),
      _eLUser = _eLBtn.ns()
    var _eDBtn = _eDept.fc(),
      _eDUser = _eDBtn.ns()
    loadBtn(_eLBtn, _eLUser, _LBtnAry, 'letter')
    loadDept(_eDBtn, _eDUser)
  }
  ...
  function loadBtn (_eBtn, _eUser, _items, _tab) {
    var _bs = new $.UI.ButtonSet({
      p: _eBtn,
      items: _items,
      itemSkin: 'Button-letter',
      onClick: function (obj) {
        onBtnClick(obj, _eUser, _tab)
      }
    })
    if (_tab == 'letter') {
      _bs.fireClick(0)
  ...
  function onBtnClick (obj, eUser, tab) {
    var _name = obj.Name,
      _args = 'm=SYS_CM_USERS&action=getUsersNoPaging&keyFields=id,uid&dataType=json',
      _sWhere = ''
    if (isNaN(+_name)) {
      if (_name == 'selAll') {
        ...
        for (var _u = 0, _uLen = _tAry.length; _u < _uLen; _u++) {
          _tAry[_u].setChecked(_ifCheck)
        }
        return
      }
      if (_name != 'ALL') {
        _sWhere = '&jsonCondition={"pinYin":"' + _name + '"}'
      }
    } else {
...
$.UI.UserSelector = function (j) {
  ...
  function onBtnClick (obj, eUser, tab) {
    ...
    $.Util.ajax({
      ...
      onSuccess: function (data) {
        ...
        users = []
        eUser.h('')
        currBtnSet = obj.ButtonSet
        for (var i = 0; i < _dLen; i++) {
          var _user = _dAry[i]
          users.push(new $.UI.Select({
            p: eUser,
            type: 'CheckBox',
            cn: 'ml3',
            css: 'min-width:75px;',
  ...
  me.setUsers = function (ids) {
    ...
    if (_count && _count == _uLen) {
      currBtnSet.setChecked('selAll', true)
    } else {
      currBtnSet.setChecked('selAll', false)
    }
  }
...
$.UI.PopDialog = function (j) {
  ...
  var me = this,
    _fn = function () {}
  var owner, scrollBar, com, temp, _dx = 16,
    mask
  var args = {
    p: $DB,
    ifFixedHeight: false,
    skin: 'PopTips-default',
    arrowBC: '#CCC',
    ifMask: false,
    arrowBBC: '#FFF',
    ePop: null,
  ...
  function layout () {
    owner = args.p.adElm('', 'div').cn(args.skin + ' ' + args.cn).css('left:0px;top:0px;' + args.css).evt('click', function (e) {
      var e = $.e.fix(e)
      e.stop()
    })
    owner.onselectstart = function () {
    ...
    scrollBar = new $.UI.ScrollBar({
      p: owner,
      ifFixedHeight: args.ifFixedHeight
    })
    scrollBar.evt('onResize', function (obj) {
      ...
      if (_ePop && h) {
        ...
        var _newX = _x - _popOwnerP.x + 1,
          _newY = _y - _popOwnerP.y + _h
        var _w_wh = $.wh(),
          _ww = _w_wh[0] * 2,
          _wh = _w_wh[1] * 2
        var _aw = w + _x,
        ...
        if (_aw > _ww) {
          _newX = _x - w + _pos.w - 22
          _dx = owner.csn('width') - 25
        }
        if (_newY < 0) {
          _newY = 0
        }
    ...
    args.owner = owner
    if (args.ifClose) {
      owner.adElm('', 'a').cn('pt-close').attr('title', '关闭').evt('click', function () {
        me.hide()
        $.UI.DestroyPopElm('--PopTips-Close')
      })
    }
    if (args.ifMask) {
      mask = new $.UI.Mask({
  ...
  me.init = function (j, ifReturnCom) {
    com = scrollBar.addElm(j)
    if (ifReturnCom) {
      return com
    }
    return me
  }
  me.html = function (sHtml) {
    scrollBar.html(sHtml)
    return me
  }
  me.clearHTML = function (ifClearStyle) {
    if ((ifClearStyle == null ? true : ifClearStyle)) {
      owner.style.cssText = ''
    }
    scrollBar.html('')
    if (temp) {
  ...
  me.setPos = function (x, y, pos) {
    var _dir = 'top'
    if (!temp) {
      temp = args.p.adElm('', 'div').cn('w16 h16 pa').css('z-index: 1001;')
    }
    if (y > pos.y) {
      _dir = 'top'
    ...
    new $.UI.Arrow({
      ...
      diff: 1,
      comMode: 'border',
      borderColor: args.arrowBC,
      backgroundColor: args.arrowBBC,
      cn: 'cp',
      css: 'padding:3px 5px;border:1px solid #FFC97C;background-color:#FFFEC9;z-index:25;max-width:320px;',
      dir: _dir
    })
...
$.UI.KeyPanel = function (j) {
  ...
  function toHtml () {
    var aNum = args.keys.split(','),
      _nNum = $.m.p(Math.sqrt(aNum.length) + 1)
    var sObj = {},
      _dKeys = args.disabledKeys.split(',')
    for (var i = 0, _len = _dKeys.length; i < _len; i++) {
      sObj[_dKeys[i]] = 1
    }
    var _w = args.p.csn('width') / _nNum - 10,
      _h = args.p.csn('height') / _nNum - 5
    var fg = $Fg(),
    ...
    aNum.ec(function (i) {
      ...
      if (sObj[_v]) {
        _key.ac('disabled').attr('title', 'disabled')
      }
    })
    return fg
  }
  me.resize = function () {
    owner.h('').appendChild(toHtml())
    return me
  }
  me.getArgs = function () {
    return args
...
$.UI.FormItem = function (j) {
  var me = this,
    owner, args, evtHandler, eInput, eHead, eBody
  me.next = null
  me.pre = null
  me.doms = null

  function setDefault (j) {
    args = $.UI.FormItem.initArgs(j.comType, j)
    args.self = me
    me.type = args.comType
    me.name = args.name
  }

  function layout () {
    var _h = $.UI.FormItem.getHtml(args),
      _p = args.p,
      _domType = 'div'
  ...
  me.focus = function () {
    ...
    if (eInput && !args.readonly) {
      eInput.focus()
    } else {
      evtHandler.fireClick()
    }
    return me
  }
  me.blur = function () {
  ...
  me.reset = function (b) {
    ...
    switch (me.type) {
      ...
      case 'CheckBox':
        ...
        break
      case 'ScanCode':
        args.ScanCode.setValue(_dT)
        break
    }
    if (eInput) {
      eInput.dc('input-error input-ok input-disabled').ac('input-normal')
      eInput.value = args.defText
    }
    return me
  }
  me.setTitle = function (v) {
    if (eHead) {
      eHead.attr('title', v)
      if (v.length > 6) {
        v = v.substr(0, 6) + '..'
  ...
  me.getVisibled = function () {
    return args.visibled
  }
  ...
  me.setIfReq = function (v) {
    if (eHead) {
      if (v) {
        eHead.abElm('', 'span').cn('req').attr('title', '必填').h('*')
      } else {
        eHead.dc('c_6').css('')
      }
    }
    ...
    switch (me.type) {
      case 'Radios':
        for (var i = 0, _rLen = args.RadioAry.length; i < _rLen; i++) {
          var _radio = args.RadioAry[i],
            _rVal = _radio.getValue()
          if (_rVal == v || +_rVal == +v) {
            evtHandler.fireClickRadio(_radio, false)
            break
          }
        }
        break
      case 'CheckBox':
        ...
        for (var i = 0, _cLen = _sAry.length; i < _cLen; i++) {
          var _sVal = _sAry[i]
          if (_sVal) {
            args.CheckBoxs[_sVal].setChecked(true)
          }
        }
        break
    }
  ...
  function setText (v) {
    ...
    switch (me.type) {
      ...
      case 'FileUploader':
        args.instanceId = v
        var _eFiles = args.eFiles
        _eFiles.h('')
        $.Util.ajax({
          args: {
            m: 'SYS_CM_FILES',
            action: 'getFilesByLink',
            link: v,
            dataType: 'json',
            jsonCondition: '{"type": 1}'
          },
          onSuccess: function (obj) {
            var _aVal = eval(obj.get(0) || '[]'),
              _aLen = _aVal.length
            for (var i = 0; i < _aLen; i++) {
              var _fObj = _aVal[i],
                _fName = _fObj.nodeName,
                _txt = _fName
              if (_txt.length > 25) {
                _txt = _txt.substr(0, 25) + '...'
              }
              ...
              $(_eFile.lastChild).evt('click', function (e) {
                var e = $.e.fix(e),
                  _e = e.t
                e.stop()
                if (_e.tagName == 'SPAN' && _e.className == 'del') {
                  me.delFile(_e)
                }
              })
            }
  ...
  me.delFile = function (_e) {
    MConfirm.setWidth(350).show('确定删除该文件?').evt('onOk', function () {
      args.value = args.value.replaceAll(_e.attr('fid') + ',', '')
      _e.pn().r()
  ...
  me.setData = function (value, text, ifSetChange) {
    ...
    if (ifSetChange != false) {
      if (args.readonly && eInput) {
        ...
        if (!_eDel) {
          _eDel = eBody.adElm('', 'a').attr('title', '清空').cn('select-clear').evt('click', function () {
            setValue('')
            setText('')
            _eDel.hide()
          })
  ...
  me.show = function () {
    return me.setVisibled(true)
  }
  me.hide = function () {
    return me.setVisibled(false)
  }
  me.remove = function () {
    owner.r()
    me = null
...
$.UI.FormGroup = function (j) {
  ...
  function bindEvent () {
    eTitle.evt('click', function (e) {
      ...
      if (_e.tagName == 'A' && _e.attr('expand')) {
        if (_e.className.trim() == 'icon-glyph-chevron-down') {
          _e.cn('icon-glyph-chevron-right')
          eFI.hide()
        } else {
          _e.cn('icon-glyph-chevron-down')
          eFI.show()
        }
      }
    })
  ...
  me.addItem = function (j) {
    var _obj = j || {},
      _item, _idx = count.getN()
    _obj.p = eFI
    if (_obj.comType == 'Select' && _obj.items) {
      var _v_t = {},
        _iLen = _obj.items.length,
        _vObj
      for (var i = 0; i < _iLen; i++) {
        _vObj = _obj.items[i] || {}
        _v_t[_vObj.value] = _vObj.text
      }
      _obj.VT = _v_t
    }
...
$.UI.Form = function (j) {
  ...
  var args = {
    ...
    extLoadFields: [],
    btnItems: [{
      name: 'FORM-SYS-SUBMIT',
      text: '提交',
      skin: 'Button-blue',
      css: 'margin-left:100px;'
    }],
    ifSubmitOriVal: false,
    ifFixedHeight: true,
    ifFocus: false,
    extSubmitVal: {},
    hidden: {},
    onItemClick: _fn,
    onItemClickBefore: _fn,
    onClick: _fn,
    onFinishInit: _fn,
    onClose: _fn,
    onChange: _fn,
  ...
  function layout () {
    ...
    args.owner = owner
    me.btnSet = new $.UI.ButtonSet({
      p: eFoot,
      items: args.btnItems,
      onClick: onClick
    })
    me.reLoadItems(args.items, {
      onSuccess: function () {
        bindEvent()
      }
    })
  ...
  function checkItem (beginFI) {
    if (beginFI) {
      ...
      if (beginFI.get('ifSubmit') && beginFI.getVisibled() && beginFI.group.getVisibled()) {
        ...
        if (_cVal) {
          var _name = beginFI.name,
            _text = beginFI.getText().toString().trim(),
            _value = beginFI.getValue()
          var _origVal = _name + '=' + _text,
            _dType = beginFI.get('dataType')
          switch (_dType) {
            case 'int':
          ...
          if (beginFI.get('isChange')) {
            data.UText[_name] = _text
            data.UValue[_name] = _value
            data.UOrigVal.push(_origVal)
          }
          return checkItem(beginFI.next)
        } else {
          beginFI.focus()
  ...
  me.check = function (ifReturn) {
    ...
    var _checkValue = checkItem(firstFI),
      _rValue = _checkValue
    if (ifReturn) {
      _rValue = [_checkValue, data]
    }
    return _rValue
  }

  me.getButton = function (v) {
    return me.btnSet.items[v]
  }
  ...
  me.submit = function () {
    ...
    var _val, _action = args.submitApi,
      _aAry = [],
      _hVal = $.Util.toArgsString(args.hidden),
      _sArgs = '',
      _infoText = '添加'
    switch (args.state) {
      case 'Insert':
        if (args.insertApi) {
          _action = args.insertApi
        }
        if (args.ifSubmitOriVal) {
          _val = data.IOrigVal.join('&')
        } else {
          _val = data.IValue
        }
        break
      case 'Update':
        _infoText = '修改'
        if (args.updateApi) {
          _action = args.updateApi
        }
        if (args.ifSubmitOriVal) {
          _val = data.UOrigVal.join('&')
        } else {
          _val = data.UValue
        }
        break
    ...
    if (!_action) {
      MTips.show('Form提交的请求API为空', 'warn')
      return
    } else {
      _action = $.Util.toArgsString(_action)
      _aAry.push(_action)
    }
    if (_hVal) {
      _aAry.push(_hVal)
    ...
    if (typeof _val === 'object') {
      for (var k in args.extSubmitVal) {
        _val[k] = args.extSubmitVal[k]
      }
    ...
    $.Util.ajax({
      ...
      onError: function (obj) {
        if (args.onSubmitError({
          ...
          MTips.show(obj.data.split('=')[1] || (_infoText + '失败'), 'error')
        }
      }
    })
  }
  ...
  function getKeyFields () {
    var _items = me.aItem,
      ...
      _kAry = []
    for (var i = 0; i < _iLen; i++) {
      var _args = _items[i].getArgs(),
        _name = _args.name,
        _trans = _args.trans,
        _alias = _args.alias || _name
      if (_args.comType == 'Date' || _args.comType == 'EndDate') {
        _alias = 'dbo.SYS_FORMAT_TIME(' + _name + ')'
      }
      if (_args.comType == 'UserSelector') {
        _trans = 'SYS_TRANS_USERS'
      }
      if (_args.comType == 'SingleUserSelector') {
        _trans = 'SYS_TRANS_USER'
      }
      _kAry.push(_alias + ' as ' + _name)
      if (_args.gtID || _args.ifTrans) {
        _trans = 'SYS_TRANS_GT'
      }
      _trans = args.trans || _trans
      if (_trans) {
        _kAry.push('dbo.' + _trans + '(' + _name + ') as ' + _name + '_trans')
      }
    }
  ...
  me.loadDataByID = function (id, onSucc, ifChange) {
    if (!id || !args.loadApi) {
      return
    };
    return loadApi(args.loadApi + '&id=' + id, onSucc, ifChange)
  }
  me.loadDataByWhereCondition = function (api, onSucc, ifChange) {
    return loadApi(api, onSucc, ifChange)
  }

  function loadApi (api, onSucc, ifChange) {
    var _succ = onSucc || _fn,
      _ifChange = ifChange || false
    $.Util.ajax({
  ...
  me.addItem = function (j) {
    ...
    if (!_name) {
      _name = 'FormItem_' + _idx
      _obj.name = _name
    }
    var _gn = _group.name || 'FORM-SYS-GROUP-DEFAULT',
      _fg = me.groups[_gn]
    var _onCG = _obj.onChange || _fn,
      _onCK = _obj.onClick || _fn
    var _onCB = _obj.onClickBefore || _fn,
      _onEP = _obj.onEnterPress || _fn
    if (!_fg) {
      _fg = me.addGroup(_group)
    }
    _obj.group = _fg
    _item = _fg.addItem(_obj)
    _item.evt('onChange', function (obj) {
      obj.Form = me
      _onCG(obj)
      args.onChange(obj)
    })
      .evt('onClick', function (obj) {
        obj.Form = me
        _onCK(obj)
        args.onItemClick(obj)
      })
      .evt('onClickBefore', function (obj) {
        obj.Form = me
        _onCB(obj)
        args.onItemClickBefore(obj)
      })
      .evt('onEnterPress', function (obj) {
        obj.Form = me
        _onEP(obj)
        if (me.check() != false) {
          me.submit()
          args.onEnterSubmit(obj)
        }
      })
    _item.form = me
    if (lastFI) {
  ...
  me.reLoadItems = function (items, cbFn) {
    ...
    var _len = items.length,
      ...
      _idx = [],
      _f = cbFn || {},
      _onSucc = _f.onSuccess || _fn,
      _onErr = _f.onError || args.onLoadFormError
    for (var i = 0; i < _len; i++) {
      var _obj = items[i] || {},
        _type = _obj.comType,
        _gtID = _obj.gtID
      if (_type == 'CheckBox' || _type == 'Radios') {
        if (_gtID) {
          _ids.push(_gtID)
          _idx.push(i)
        }
    ...
    $.Util.ajax({
      ...
      onSuccess: function (obj) {
        ...
        initItems(items)
        _onSucc(_cbObj)
        args.onLoadFormSuccess(_cbObj)
      },
      onError: function (d) {
        MTips.show('加载CheckBox或Radios中的数据出错!', 'error')
        _onErr()
      }
    })
    return me
  ...
  me.reset = function () {
    for (var i = 0, _iLen = me.aItem.length; i < _iLen; i++) {
      me.aItem[i].reset()
    }
    return me
  }
  ...
  me.setExt = function (key, value) {
    args.extSubmitVal[key] = value
    return me
  }
  me.setHidden = function (key, value) {
    args.hidden[key] = value
    return me
  }
  me.getHidden = function (key) {
    return args.hidden[key]
  }
  me.set = function (key, value) {
    args[key] = value
    return me
...
$.UI.Paging = function (j) {
  ...
  var args = {
    p: $DB,
    buttonSkin: 'Button-paging',
    skin: 'Paging-default',
    cn: '',
    css: '',
    pageSizeAry: [5, 10, 20, 30, 40],
    pageSize: 20,
    pageIndex: 1,
    totalCount: 250,
    pages: 5,
  ...
  function setDefault (j) {
    args = $.Util.initArgs(j, args)
    _pLen = args.pages + 1
    var _bSkin = args.buttonSkin,
      _psa = args.pageSizeAry
    for (var j = 0, _jLen = _psa.length; j < _jLen; j++) {
      var _d = _psa[j]
      _dAry.push({
        name: _d,
        value: _d,
    ...
    btnAry.push({
      name: 'First',
      title: '首页',
      icon: 'icon-glyph-fast-backward',
      skin: _bSkin
    })
    btnAry.push({
      name: 'Pre',
      title: '上一页',
      icon: 'icon-glyph-backward',
      skin: _bSkin
    })
    ...
    btnAry.push({
      name: 'Next',
      title: '下一页',
      icon: 'icon-glyph-forward',
      skin: _bSkin
    })
    btnAry.push({
      name: 'Last',
      title: '尾页',
      icon: 'icon-glyph-fast-forward',
      skin: _bSkin
    })
    btnAry.push({
      name: 'Refresh',
  ...
  function layout () {
    ...
    var _d1 = _eExt.fc(),
      ...
      _d4 = _d3.ns(),
      _d5 = _d4.ns()
    _d4.evt('click', function (e) {
      onChangeClick(e)
    })
    _d1.evt('keyup', function (e) {
      var e = $.e.fix(e),
        _e = e.t
      if (e.code == 13) {
        me.forwardPage(_e.value)
      };
    })
    attr.eInputIdx = _d1
    attr.eIdx = _d2
  ...
  function onChangeClick (e) {
    ...
    e.stop()
    if ($.global.popTips) {
      $.global.popTips.remove()
    }
    $.global.popTips = new $.UI.PopDialog({
      p: $DB
    })
    $.global.popTips.clearHTML().set('ePop', _e).init({
      type: 'Menu',
      checkedValue: _e.ps().value,
      items: _dAry
    }).show()
      .evt('onClick', function (obj) {
        var _size = +obj.Value
  ...
  function onBtnClick (obj) {
    ...
    switch (_name) {
      ...
      case 'Next':
        args.pageIndex++
        if (args.pageIndex > attr.pageCount) {
          args.pageIndex--
          return
        }
        _pIdx = args.pageIndex
        if (btnIdx == args.pages) {
          attr.ifChange = true
          btnIdx = 1
        } else {
          btnIdx++
  ...
  me.forwardPage = function (v, ifExecFn) {
    ...
    if (attr.ifChange) {
      var _dVal = $.m.p((v - 1) / args.pages),
        _tVal = (_dVal + 1) * args.pages,
        _rSize = args.pages
      if (_tVal > attr.pageCount) {
        _rSize = attr.pageCount - _dVal * args.pages
      }
  ...
  function setBtnText (dVal, _rSize) {
    ...
    for (var i = 1; i < _pLen; i++) {
      ...
      if (i < _rSize + 1) {
        btn.setVisibled(_name, true)
      } else {
        btn.setVisibled(_name, false)
      }
    }
  }
  ...
  me.setTotal = function (v) {
    ...
    var _pSize = args.pageSize,
      _pCount = Math.ceil(v / _pSize)
    attr.eIdx.h('/' + _pCount + '页')
    attr.eSize.h('/' + v + '条')
    attr.pageCount = _pCount
    attr.total = v
    setPageSize(_pSize)
    attr.ifChange = true
...
$.UI.Button = function (j) {
  ...
  function setDefault (j) {
    args = $.Util.initArgs(j, args)
    _skin = args.skin
    cn = _skin + ' ' + _skin + '-normal  ' + args.cn
    css = args.css
    if (args.type == 'menu' || args.isMenu) {
      args.ifClose = false
    }
    if (args.width) {
  ...
  function layout () {
    ...
    eText = eIcon.ns()
    if (args.ifVertical) {
      eText.css('width: 16px;word-wrap: break-word;')
    }
  ...
  function findTarget (_e) {
    ...
    var _t = _e.className.trim(),
      _eLi, _tn = _e.tagName
    if (_tn == 'SPAN') {
      return findTarget(_e.pn())
    }
    if (_tn == 'A') {
      return {
        target: _t,
    ...
    owner.evt('click', function (e) {
      ...
      switch (_obj.target) {
        case 'close':
          ...
          break
        default:
          if (_type == _obj.target) {
            clickMenu(_obj.eLi)
          } else {
            $.UI.DestroyPopElm('Button-Click')
          }
          if (_type == 'toggle') {
            var _ifPress = !args.ifPress
  ...
  function clickMenu (eLi) {
    ...
    var _mArgs = args.MArgs || {},
      _pTips = $.global.popTips
    _mArgs.type = 'Menu'
    ...
    _pTips.set('ePop', eLi).show().init(_mArgs, true)
      .evt('onClick', function (obj) {
        obj.Button = me
        obj.BtnName = args.name
        obj.BtnArgs = args
        obj.PopTips = _pTips
        if (args.onMenuClick(obj) != false) {
          _pTips.hide()
        }
      })
  }
  ...
  me.fireClick = function (cbFn, ifPress) {
    var _f = cbFn || args.onClick,
      _ifPress = ifPress == null ? true : ifPress
    if (!args.enabled) {
      return me
    }
    if (args.type == 'toggle') {
      _ifPress = !args.ifPress
    }
    if (args.type == 'normal') {
      _ifPress = false
    }
  ...
  me.setFocus = function (v) {
    if (v != null && args.enabled) {
      args.ifFocus = v
      if (v) {
        owner.dc(_skin + '-normal').ac(_skin + '-focus')
      } else {
        owner.dc(_skin + '-press btn-focus').ac(_skin + '-normal')
      }
    }
    return me
  }
  me.setPress = function (v) {
    if (v != null && args.enabled) {
      args.ifPress = v
      if (v) {
        owner.dc(_skin + '-normal').ac(_skin + '-press')
        args.onPress({
          Button: me
        })
      } else {
        owner.dc(_skin + '-press').ac(_skin + '-normal')
      }
    }
    return me
  }
...
$.UI.ButtonSet = function (j) {
  ...
  function loadApi (succ) {
    if (args.loadApi) {
      me.loadApi(args.loadApi, false, args.btnType, succ)
    } else {
      succ()
    };
  }

  function loadGT (succ) {
    if (args.gtID != null) {
      me.loadApi('m=SYS_TABLE_BASE&table=SYS_CM_GLOBAL_TABLE&action=getRightsListByPid&pid=' + args.gtID, false, args.gtType, succ, 'nodeName as text, nodeName as nn, id as name, icon')
    } else {
      succ()
    }
  }

  function loadGBS (succ) {
    if (args.gbsID != null) {
      me.loadApi('m=SYS_CM_USERS&action=getRightsToolBars&pid=' + args.gbsID, false, args.gbsType, succ, 'nodeName as text, nodeName as nn, id as name, icon, btnType as type')
    } else {
      var _succ = succ || args.onSuccess
      _succ({
        ButtonSet: me,
        items: me.aItem
      })
  ...
  me.addItem = function (j, ifReturn) {
    var _j = j || {},
      _ifR = ifReturn || false,
      _onClick = _j.onClick || _fn
    _j.p = owner
    _j.onClick = function (obj) {
      obj.ButtonSet = me
      if (obj.Args.type == 'tab') {
        me.setSelTabItem(obj.Button)
      };
      _onClick(obj)
      args.onClick(obj)
    }
    _j.onMenuClick = function (obj) {
      obj.ButtonSet = me
      return args.onMenuClick(obj)
    }
    _j.onMouseDown = function (obj) {
      obj.ButtonSet = me
      return args.onMouseDown(obj)
    }
    if (!_j.skin) {
      _j.skin = args.itemSkin
    }
    if (!_j.align) {
      _j.align = args.itemAlign
    }
    var _btn = new $.UI.Button(_j)
    _btn.evt('onClose', function (obj) {
      obj.ButtonSet = me
      return args.onClose(obj)
    })
  ...
  me.setAllEnabled = function (v) {
    ...
    for (var i = 0, _len = me.aItem.length; i < _len; i++) {
      me.aItem[i].setEnabled(v)
    }
  }
  ...
  me.setSelTabItem = function (v) {
    var _item = me.getItem(v)
    if (_item) {
      var _tab = _item.getArgs().tab,
        _selItem = me.selTabItem[_tab]
      if (_selItem == _item) {
        return me
      }
      if (_selItem) {
        _selItem.setPress(false)
      }
      _item.setPress(true)
  ...
  me.loadApi = function (api, ifReLoad, type, onSucc, fields) {
    ...
    $.Util.ajax({
      ...
      onSuccess: function (data) {
        var _sAry = data.get(0),
          _iAry = [],
          _onSucc = onSucc || args.onSuccess
        if (_sAry) {
          _iAry = eval(_sAry)
        }
        if (ifReLoad != false) {
  ...
  me.fireClick = function (k, f, ifPress) {
    var _item = me.getItem(k)
    if (_item) {
      _item.fireClick(f, ifPress)
    }
    return me
  }
  me.loadItems = function (items, type) {
    for (var i = 0, _iLen = items.length; i < _iLen; i++) {
      var _iObj = items[i]
      _iObj.type = type || _iObj.type
      me.addItem(_iObj)
    };
    return me
  }
  me.reLoadItems = function (items) {
    me.clear()
    return me.loadItems(items)
  }
  me.setIcon = function (k, v) {
    var _item = me.getItem(k)
    if (_item) {
      _item.setIcon(v)
    }
  ...
  me.getText = function (v) {
    var _item = me.getItem(v)
    if (_item) {
      return _item.getText()
    }
  }
  me.setText = function (k, v) {
    var _item = me.getItem(k)
    if (_item) {
      _item.setText(v)
    }
...
$.UI.Menu = function (j) {
  ...
  function layout () {
    ...
    for (var i = 0, _iLen = args.items.length; i < _iLen; i++) {
      addItem(args.items[i])
    }
    if (args.gtID) {
      args.loadApi = 'm=SYS_TABLE_BASE&table=SYS_CM_GLOBAL_TABLE&action=getRightsListByPid&pid=' + args.gtID
    }
    if (args.loadApi) {
      me.loadAjax({
        args: args.loadApi
  ...
  function addItem (j) {
    var _obj = $.Util.initArgs(j, iArgs),
      _idx = count.getN(),
      _name = _obj.name || 'MENU_' + _idx
    var _eLI = owner.adElm('', 'li'),
      _iType = (_obj.type || 'item').trim()
    if (_iType == 'item') {
  ...
  function bindEvent () {
    owner.evt('click', function (e) {
      ...
      if (_eLi) {
        var _val = _eLi.attr('_value'),
          _eTxt = _eLi.chn(2),
          _txt = _eTxt.ht(),
          _name = _eLi.attr('_name'),
          _style = '',
          _temp = _eTxt.fc()
        if (me.selLi) {
          me.selLi.dc('selected')
        }
        _eLi.ac('selected')
        me.selLi = _eLi
        if (_temp.tagName == 'FONT' || _temp.tagName == 'COLOR') {
          if (_temp.attr('color')) {
            _style += 'color:' + _temp.attr('color') + ';'
          };
          if (_temp.attr('style')) {
            _style += _temp.attr('style')
          };
        }
        args.onClick({
          Menu: me,
  ...
  me.loadAjax = function (obj) {
    var _obj = obj || {},
      _args = _obj.args || '',
      _f = _obj.cbFn || {}
    if (_args && args.onAjax({
      Menu: me,
      Args: _args
    }) != false) {
      var _onSuc = _f.onSuccess || args.onSuccess,
        _onErr = _f.onError || args.onError
      var _tK = args.textKey,
        _vK = args.valueKey,
        _kv = _vK + ' as value, ' + _tK + ' as text'
      if (args.extFields.length) {
        _kv += ',' + args.extFields.join(',')
      }
      if (args.ifShowIcon) {
        _kv += ',icon'
      }
  ...
  function onLoadOver () {
    if (args.checkedValue) {
      var sel = itemsObj[args.checkedValue]
      if (sel) {
        me.setSelected(sel)
      }
    }
...
$.UI.TreeList = function (j) {
  ...
  var args = {
    ...
    loadApi: '',
    updateApi: '',
    css: 'border-top:1px solid #E4E2E2;',
    cn: '',
    skin: '',
    style: 'tree:nodeName',
    onSuccess: _fn,
  ...
  function layout () {
    ...
    var _lArgs = $.Util.initArgs({
      p: owner,
      loadApi: attr.loadApi,
  ...
  function bindEvent () {
    me.List.evt('onTDClick', function (obj) {
      obj.TreeList = me
      disposeChild(me.next)
      if (me.next) {
        me.next.remove()
        me.next = null
      }
      if (args.lockLevel == args.currLevel) {
        args.onTDClick(obj)
        return
      }
      if (args.style.indexOf('tree') == -1) {
        var _eTR = obj.Target.eTr,
          _pid = _eTR.attr('rowId') || _eTR.attr('id'),
          _sons = +_eTR.attr('sons')
        if (!_sons) {
          args.onTDClick(obj)
          return
        }
        var _pPos = args.p.pos(),
          _trPos = _eTR.pos()
        var _x = _trPos.x - _pPos.x + _trPos.w,
          _y = 0,
          _loadApi = $.JSON.decode($.JSON.encode(attr.loadApi))
        if (args.style == 'menu') {
          _y = _trPos.y - args.p.pos().y
        }
        _loadApi.pid = _pid
        var _nArgs = $.Util.initArgs({
          loadApi: _loadApi,
          x: _x,
          y: _y,
          currLevel: args.currLevel + 1
        }, args)
      ...
      args.onTDClick(obj)
    }).evt('onTDClickBefore', function (obj) {
      if (args.onTDClickBefore(obj) == false) {
        return false
      }
    })
  }
  ...
  me.refresh = function (cbFn, ifCheckSel, idc) {
    if (!ifCheckSel) {
      ifCheckSel = false
    };
    me.List.refresh(cbFn, ifCheckSel)
    if (idc) {
      disposeChild(me.next)
    }
  } // idc: ifDisposeChilds的简称
  me.getArgs = function () {
    return args
  }
  me.evt = function (key, fn) {
...
$.UI.Layout = function (j) {
  ...
  function layout () {
    ...
    if (dir == 'ns') {
      ...
      if (dirLock == 1) {
        ...
        if (ifDrag) {
          $.drag.init(eBar, null, null, null, min, max, 1)
        }
      } else {
        _ddir = 's'
        ...
        if (ifDrag) {
          $.drag.init(eBar, null, null, null, pH - max, pH - min, 1)
        }
      }
    } else {
      ...
      if (dirLock == 1) {
        ...
        if (ifDrag) {
          $.drag.init(eBar, null, min, max, null, null, null, 1)
        }
      } else {
        _ddir = 'e'
        ...
        if (ifDrag) {
          $.drag.init(eBar, null, pW - max, pW - min, null, null, null, 1)
        }
      }
    }
    if (args.ifCover) {
      eDirFlag.dc('dn').ac('dir-' + _ddir).attr('dir', _ddir)
    }
  }
  ...
  function bindEvent () {
    ...
    if (ifDrag) {
      ...
      eBar.evt('click', function (e) {
        ...
        if (_barDir) {
          me.cover4Dir(_barDir, +_e.attr('state'))
        }
      })
    }
    owner.evt('click', function (e) {
      var e = $.e.fix(e),
        _e = e.t
      $.UI.DestroyPopElm('Layout-owner-click')
    })
  }
  ...
  function resizeScrollBar (p) {
    ...
    if (hRate < 100) {
      hBar.css('width:' + hRate + '%;')
      eHBar.dc('dn')
    } else {
      eHBar.ac('dn')
    }
    if (vRate < 100) {
      vBar.css('height:' + vRate + '%;')
      eVBar.dc('dn')
    } else {
      eVBar.ac('dn')
    }
  ...
  me.cover4Dir = function (dir, v) {
    var _ph = $.m.p(p.offsetHeight),
      _pw = $.m.p(p.offsetWidth)
    switch (dir) {
      case 'n':
        if (v) {
  ...
  me.resize = function (f) {
    ...
    var _hAry = eHead.find('div:com=ScrollBar'),
      _fAry = eFoot.find('div:com=ScrollBar')
    for (var i = 0, _len = _hAry.length; i < _len; i++) {
      resizeScrollBar(_hAry[i])
    }
    for (var i = 0, _len = _fAry.length; i < _len; i++) {
      resizeScrollBar(_fAry[i])
    }
  }

  me.resizeInter = function (s, f) {
    var _nsAry = $(s).find('div:mType=NS'),
      _weAry = $(s).find('div:mType=WE')
...
$.UI.BaseDiv = function (j) {
  ...
  me.setBody = function (v, slip, cbFn) {
    var _h = head_h + v,
      ...
      _f = cbFn || function () {}
    if (_s) {
      _base.ease(['height'], [_h], 600, 1, {})
    } else {
      ;
      _base.css('height:' + _h + 'px;')
    }
    _f(me, _base)
...
$.UI.BaseDivH = function (j) {
  ...
  function layout () {
    var _oBase = args.skin.base,
      _oHead = args.skin.head,
      _oBody = args.skin.body,
      _oFoot = args.skin.foot
    var baseCn = _oBase.cn || '',
      baseCss = _oBase.css || '',
      headCn = _oHead.cn,
      headCss = $.box(',,' + args.head_h + ',') + _oHead.css,
      bodyCn = _oBody.cn,
      bodyCss = _oBody.css,
      footCn = _oFoot.cn,
      footCss = $.box(',,' + args.foot_h + ',') + _oFoot.css
...
$.UI.Arrow = function (j) {
  ...
  var j = j || {},
    cn = j.cn || '',
    css = j.css || '',
    comMode = j.comMode || 'normal',
    diff = j.diff || 3
  var p = $(j.p || $DB),
    dir = j.dir || 'bottom' // top, left, right, bottom
  var bc = j.borderColor || '#bbbbbb',
    bgc = j.backgroundColor || '#ffffff',
    _w = j.w || 10
  var box = j.box || {
    blw: _w,
    btw: _w,
  ...
  var _blw = box.blw || box.w1,
    _btw = box.btw || box.w2,
    _brw = box.brw || box.w3,
    _bbw = box.bbw || box.w4
  var _blc = box.blc || box.c1,
    _btc = box.btc || box.c2,
    _brc = box.brc || box.c3,
    _bbc = box.bbc || box.c4
  var _bls = _blc ? 'solid' : 'dashed',
    _bts = _btc ? 'solid' : 'dashed'
  var _brs = _brc ? 'solid' : 'dashed',
    _bbs = _bbc ? 'solid' : 'dashed'
  var html = ''
  switch (comMode) {
    case 'normal':
    ...
    case 'border':
      ...
      if (dir == 'bottom' || dir == 'top') {
        bw = $.m.p(p.csn('width') / 2)
      }
      if (dir == 'left' || dir == 'right') {
        bw = $.m.p(p.csn('height') / 2)
      }
      var _cCss = ';+overflow:hidden;border-width:' + bw + 'px;',
        _s1 = '',
        _s2 = ''
      switch (dir) {
        case 'bottom':
...
$.UI.ArrowTips = function (j) {
  ...
  function layout () {
    ...
    var _dirCss = 'width:' + arrSize + 'px;height:' + arrSize + 'px;',
      _dirH = ''
    switch (dir) {
      case 'top':
    ...
    var _aBC = $S(owner).borderColor,
      _aBGC = $S(owner).backgroundColor
    if (!_aBC) {
      _aBC = owner.cs('border-color')
    }
    if (_aBGC) {
      _aBGC = owner.cs('borderground-color')
    }
    new $.UI.Arrow({
      p: eDir,
      diff: 2,
...
$.UI.MiniTips = function (j) {
  ...
  var args = {
    ...
    arrowSize: 13,
    cn: 'r3 fs12',
    css: 'padding:3px 5px;border:1px solid #FFC97C;background-color:#FFFEC9;'
  }
...
$.UI.ResizeTool = function (p) {
  ...
  var resizeDiv_e_mousedown = function (e) {
    ...
    dragFlag.onDrag = function (a) {
      ...
      var _newW = _newPos.x - _pX + 12,
        _newH = _newPos.y - _pY + 12
...
$.UI.Resizer = function (j) {
  ...
  owner.evt('mousedown', function (e) {
    ...
    owner.onDrag = function (a) {
      if (!$.global.dragMask) {
        $.global.dragMask = new $.UI.Mask({
          p: $DB,
          alpha: 0,
          cn: 'cnw'
        })
      } else {
        $.global.dragMask.show()
      }
      var _flagPos = owner.pos(),
        _pPos = p.pos()
      var _newW = _flagPos.x - _pPos.x + 14,
        _newH = _flagPos.y - _pPos.y + 14
      if (_newW < minW) {
        _newW = minW
      }
      if (_newH < minH) {
    ...
    owner.onDragEnd = function () {
      $S(p).zIndex = _z_index
      if ($.global.dragMask != null) {
        $.global.dragMask.hide()
      }
      if (_selfMask) {
        _selfMask.remove()
        _selfMask = null
...
$.UI.Tips = function (j) {
  ...
  var args = {
    ...
    comMode: 0,
    skin: 'Tips-default',
    toolBarSkin: 'Button-toolbar-icon',
    lifeTime: 0,
    toolBarAry: [],
    ifDrag: false,
    ifFixedHeight: true,
    ...
    maskOwner: null,
    maskAlpha: 10,
    onToolBarMouseDown: _fn,
    onToolBarMenuClick: _fn,
    onClose: _fn,
    onClick: _fn,
    onResize: _fn,
    onResizeEnd: _fn,
  ...
  function setDefault (j) {
    args = $.Util.initArgs(j, args)
    if (!args.maskOwner) {
      args.maskOwner = args.p
    }
  }

  function layout () {
    var eHead, eFoot, _pos = getXY(args.comMode),
      _hHtml = '<a style="float:left;margin:' + (args.head_h - args.icon_h) / 2.5 + 'px 3px;"></a><span style="float:left;line-height:30px;"></span>'
    if (args.ifFixedHeight) {
      var _bd = new $.UI.BaseDiv({
        p: args.p,
        head_h: args.head_h,
        foot_h: args.foot_h,
        cn: args.skin + ' ' + args.cn
      })
      owner = _bd.base
      eHead = _bd.head
      eBody = _bd.body
      eFoot = _bd.foot
      eHead.ac('Tips-head').h(_hHtml)
      eBody.ac('Tips-body')
      eFoot.ac('Tips-foot')
    } else {
      var _sCss = 'height:{0}px;line-height:{0}px;',
        _sHtml = '<div class="Tips-head oh" style="{0}">{1}</div><div class="Tips-body"></div><div class="Tips-foot" style="{2}"></div>'
    ...
    if (args.ifResize) {
      resizeDiv = new $.UI.Resizer({
        ...
        onDragEnd: function (_m, _a, _b) {
          args.onResizeEnd(me, _m, _a, _b)
        }
      })
    };
    owner.onselectstart = function () {
    ...
    if (args.ifClose) {
      owner.abElm('', 'a').cn('tips-close').attr('title', '关闭').evt('click', function () {
        if (args.onClose(me) != false) {
          me.removeAni()
        };
      })
    }
  ...
  function onMaskClick () {
    if ($('').split(',')[0].toLow() == 'safari') {
      ...
      var _a = setInterval(function () {
        if (me && me.base) {
          me.base.ac('tips_focus')
        }
      }, 10)
      setTimeout(function () {
        clearInterval(_a)
        if (me && me.base) {
          me.base.dc('tips_focus')
        }
      }, 400)
    }
  }
  ...
  function toolBarLayout (p) {
    ...
    if (args.ifExpand) {
      _ary.push({
        name: 'TIPS-SYS-EXPAND',
        icon: 'icon-compact-bold-b',
        title: '展开'
      })
    }
    if (args.ifFullScreen) {
      _ary.push({
        name: 'TIPS-SYS-FULLSCREEN',
        icon: 'icon-glyph-zoom-in',
        title: '全屏'
      })
    }
    _ary = _ary.concat(args.toolBarAry)
    me.toolBar = new $.UI.ButtonSet({
      p: p,
      loadApi: args.toolBarLoadApi,
      gtID: args.gtID,
      gtType: args.gtType,
      gbsID: args.gbsID,
      gbsType: args.gbsType,
      ifRights: args.ifRights,
      items: _ary,
      onClick: onToolBarClick,
      onSuccess: args.onLoadToolBarSuccess,
      onMouseDown: args.onToolBarMouseDown,
      onMenuClick: args.onToolBarMenuClick,
      itemAlign: 'right',
      itemSkin: args.toolBarSkin,
      skin: args.btnSetSkin
    })
  }
  ...
  function getXY (comMode) {
    ...
    switch (comMode) {
      case 1:
        ...
        break
      case 2:
        var _eMO = args.maskOwner,
          _mW = _eMO.csn('width') / 2,
          _mH = _eMO.csn('height') / 2
        x = _mW - w / 2
        y = $.m.p(_mH - h / 1.5)
        break
      case 3:
  ...
  me.setWH = function (width, height) {
    if (width && height) {
      args.width = width
      args.height = height
      owner.css('width:' + width + 'px;height:' + height + 'px;border:1px solid #cccccc;')
    }
    return me
  }
  me.setPos = function (x, y) {
    var _pos = ''
    if (x == 'auto') {
      _pos += 'left:50%;margin-left:-' + args.width / 2 + 'px;'
    } else {
      _pos += 'left:' + x + 'px;'
    }
    if (y == 'auto') {
      _pos += 'top:50%;margin-top:-' + args.height / 2 + 'px;'
    } else {
      _pos += 'top:' + y + 'px;'
    }
    owner.css(_pos)
  ...
  me.setContent = function (v) {
    eBody.h(v)
    if (!args.width && v) {
      owner.css('margin-left:-' + (eBody.csn('width') / 2) + 'px;')
    }
    return me
  }
  me.setZIndex = function (v) {
    if (v != null || v) {
      owner.css('z-index:' + v)
    }
    return me
  ...
  me.removeAni = function (t) {
    owner.alpha(100).ease(['alpha', 'top'], [0, owner.csn('top') - 20], t || 400, 1, {
      e: function () {
        if (me && me.remove) {
          me.remove()
        }
...
$.UI.PopTips = function (j) {
    ...
    switch (args.type) {
      ...
      case 'tips':
        tips = new $.UI.Tips({
          ...
          comMode: 'x-auto',
          y: 10,
          css: 'padding: 8px 15px;font-size: 14px;color: #fff;text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.2);z-index: 1001;'
        })
        break
      case 'confirm':
        args.head_h = 30
        args.foot_h = 40
        args.css = 'z-index:1100;'
        args.title = '确认对话框'
        args.comMode = 'x-auto'
        args.y = 250
        args.ifFixedHeight = true
  ...
  function setTipsContent (text, state) {
    ...
    switch (state) {
      case 'ok':
      case 'pass':
        _sCss = 'background-color: #00cc33;border:1px solid #00CC33;'
        break
      case 'error':
        _sCss = 'background-color: #f00;border:1px solid #FC0808;'
        break
      case 'warn':
        _sCss = 'background-color: #ff7800;border:1px solid #FF7800;'
        break
      case 'loading':
        _sCss = 'background-color: #3EABFF;border:1px solid #3EABFF;'
        break
      default:
        _sCss = 'background-color: #3EABFF;border:1px solid #3EABFF;'
        break
  ...
  me.show = function (text, state) {
    ...
    if (text != null) {
      switch (args.type) {
        ...
        case 'tips':
          tips.show()
          setTipsContent(text, state)
          setTimeout(function () {
            me.hide()
          }, 2000)
          break
        case 'confirm':
          tips.body.fc().chn(1).h(text)
          tips.base.css('z-index:1100;')
          tips.base.ns().css('z-index:1009;')
          tips.show()
          break
      }
    }
  ...
  me.load = function (value) {
    ...
    switch ($.getType(_val)) {
      ...
      case 'object':
        for (var k in _val) {
          var _v = _val[k],
            _eI = eContent.adElm('', 'div').cn('fl wp')
          switch (typeof _v) {
            case 'object':
              new $.UI.Json({
                p: _eI,
...
$.UI.Panel = function (j) {
  ...
  function layout () {
    var _cn = args.skin + ' ' + args.cn,
      _css = args.css
    if (args.width) {
      _css += 'width:' + args.width + 'px;'
    }
    if (args.height) {
      _css += 'height:' + args.height + 'px;'
    }
    if (args.ifFixedHeight) {
      _cn += ' hp'
    } else {
  ...
  me.expand = function () {
    ...
    if (_cs.indexOf('unfold') != -1) {
      eIcon.dc('unfold').ac('fold')
      eContent.hide()
    } else {
      eIcon.dc('fold').ac('unfold')
      eContent.show()
    }
...
$.UI.AttrPanel = function (j) {
  ...
  function layout () {
    ...
    if (args.ifClose) {
      _expandIdx++
      _iAry.unshift({
        name: 'ATTRPANEL-SYS-CLOSE',
        icon: 'icon-glyph-remove',
        title: '关闭'
      })
    }
    ...
    var _eLeg = owner.fc().fc().css('height: ' + args.titleHeight + 'px;line-height: ' + args.titleHeight + 'px;').evt('click', function (e) {
      clickHead(e)
    })
    eIcon = _eLeg.fc()
    eTitle = eIcon.ns()
  ...
  function clickHead (e) {
    ...
    switch (_key) {
      case 'ATTRPANEL-SYS-EXPAND':
        me.setFold(+_e.attr('state'), _e)
        break
      case 'ATTRPANEL-SYS-CLOSE':
        me.hide()
        break
    }
    args.onToolBarClick({
  ...
  me.setFold = function (v, _e, ifExec) {
    if (!_e) {
      if (_expandIdx != 1) {
        _e = eIcon.pn().chn(_expandIdx)
      } else {
        return me
      }
    }
    if (v) {
      _e.dc('icon-glyph-chevron-down').ac('icon-glyph-chevron-right').attr('state', 0)
      eContent.show()
    } else {
      _e.dc('icon-glyph-chevron-right').ac('icon-glyph-chevron-down').attr('state', 1)
      eContent.hide()
    }
    if (ifExec != false) {
      args.onExpand({
...
$.UI.Mask = function (j) {
  ...
  function layout () {
    ...
    _owner.evt('click', function (e) {
      var e = $.e.fix(e)
      e.stop()
      args.onClick(me)
      $.UI.DestroyPopElm('Mask--Click')
    })
    if (isIE) {
      _owner.h('<iframe frameBorder=0 frameBorder="no" allowTransparency="true" border="0" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" style="border-style: none" class="pa bc_0 z-1 dn" srclayout="blank.html"></iframe>')
    }
    me.maskDiv = _owner
...
$.UI.Calendar = function (j) {
  ...
  var args = {
    ...
    minYear: 1910,
    maxYear: 2200,
    skin: 'Calendar-default',
    selCn: 'td-select',
    nowCn: 'td-now',
    disabledCn: 'td-disabled',
    nowD: '',
    selDate: '',
    onClick: _fn,
    onClickBefore: _fn,
  ...
  function setDefault (j) {
    ...
    args.disabledObj = {}
    args.abledObj = {}
    for (var di = 0, _diLen = args.disabledDate.length; di < _diLen; di++) {
      args.disabledObj[args.disabledDate[di]] = true
    }
    for (var ai = 0, _aiLen = args.enabledDate.length; ai < _aiLen; ai++) {
      args.abledObj[args.enabledDate[ai]] = true
      _ifEnPart = true
    }
  }

  function layout () {
    var _base = new $.UI.BaseDiv({
      p: args.p,
      ifFixedHeight: true,
      head_h: args.head_h,
      foot_h: args.foot_h,
      skin: 'BaseDiv-Calendar'
    })
    eBody = _base.body
    fillBody(args.nowD)
    fillHead(_base.head)
    fillFoot(_base.foot)
    if (args.ifLockDay) {
      eBody.adElm('', 'div').cn('lockDay').h('<span>日期已被锁定, 请选择时分秒!</span><div></div>')
    }
  }
  ...
  function getHtmlByDate (date) {
    var nowD = new Date(),
      nowM, dAry, fD = new Date(),
      sD = new Date(),
      _now_D = new Date(),
      fDWeek, i, d, s1 = [],
      s2 = []
    var _initTDH = $.m.p((args.p.csn('height') - (args.head_h + args.foot_h + 35)) / 6),
      _initTDW = $.m.p((args.p.csn('width') / 7)),
      isC = false
    var dStyleAry = args.dStyleAry
    if (args.skin == 'Calendar-container') {
      isC = true
    }
    nowD = date ? nowD.str2Date(date) : nowD
    me.nowD = nowD
    nowM = nowD.getMonth()
    dAry = nowD.date2Str().split(' ')[0].split('-')
    dAry[2] = 1
    ...
    for (d = 0; d < 6; d++) {
      ...
      for (i = 0; i < 7; i++) {
        ...
        var _d = sD.date8('-'),
          c = (dStyleAry[_d] ? dStyleAry[_d] : 'cp'),
          cNowDStyle = '',
          cSelDStyle = ''
        if (sD.getMonth() != nowM) {
          c = args.disabledCn + ' cna'
        } else {
          if (selDate && sD.date8('-') == selDate) {
            cSelDStyle = ' ' + args.selCn
          }
        }
        if (_ifEnPart) {
          if (args.abledObj[_d]) {
        ...
        if (_selectDate == _nowDate[0]) {
          if (nowD.getMonth() == sD.getMonth()) {
            cNowDStyle = ' ' + args.nowCn + ' '
          }
        }
    ...
    for (i = 0; i < 7; i++) {
      s2.push('<tr>' + (i == 0 ? '<th>' + args.hStyle.join('</th><th>') + '</th>' : s1[i - 1]) + '</tr>')
    }
    var st = '<div class="Calendar ' + args.skin + '"><table>' + s2.join('') + '</table></div>'
    return st
  }

  function hidePopMYTips () {
    if ($.global.popMYTips) {
      $.global.popMYTips.hide()
    }
  }

  function popMYTips (_e, items, fn) {
    if ($.global.popMYTips) {
      $.global.popMYTips.remove()
    }
    $.global.popMYTips = new $.UI.PopDialog({
      p: $DB,
      css: 'min-width:60px;max-height:180px;'
    })
    var _pTips = $.global.popMYTips,
      _f = fn || _fn
    _pTips.set('ePop', _e).hide().init({
      type: 'Menu',
      items: items
    }).show().evt('onClick', function (obj) {
      _f(obj, _pTips)
      _pTips.hide()
    })
  }
  ...
  function fillHead (eHead) {
    var _iconCn = 'cal-icon',
      _year = me.nowD.getFullYear(),
      _month = me.nowD.getMonth() + 1
    if (args.ifLockYM) {
      _iconCn = 'cal-icon-hidden'
    }
    ...
    eHead.evt('click', function (e) {
      ...
      switch (_rCn.trim()) {
        ...
        case 'yCenter':
          ...
          popMYTips(_e, attr.yearItems, function (obj) {
            me.setYM(obj.Name)
          })
          break
        case 'yRight':
        ...
        case 'mCenter':
          if (!attr.monthItems) {
            attr.monthItems = []
            for (var i = 1; i < 13; i++) {
              attr.monthItems.push({
                name: i,
                text: i
              })
            }
          }
          popMYTips(_e, attr.monthItems, function (obj) {
            me.setYM(null, obj.Name)
          })
          break
        case 'mRight':
          var _m = +_eM.attr('month') + 1,
  ...
  function fillBody (date) {
    var _sData = date || me.nowD
    eBody.h(getHtmlByDate(_sData))
    eTable = eBody.fc().fc()
  }
  ...
  me.setYM = function (year, month, day) {
    var _y = year || me.nowD.getFullYear(),
      _m = month || me.nowD.getMonth() + 1,
      _d = day || me.nowD.getDate()
    var _ymdStr = _y + '-' + _m + '-' + _d
    _eY.attr('year', _y).h(_y + '&nbsp;年')
    _eM.attr('month', _m).h(_m + '&nbsp;月')
    fillBody(_ymdStr)
    args.onYMClick(_ymdStr)
  }
  me.getCurrYM = function () {
    var _y = _eY.attr('year'),
      _m = _eM.attr('month')
    return [_y, _m]
  }
  me.resize = function () {
    var _pH = me.p.csn('height'),
      _tdH = $.m.p((_pH - _foot_h - 61) / 6)
    var _pW = me.p.csn('width'),
      _tdW = $.m.p(_pW / 7)
    var _tds = _body.find('td'),
      _len = _tds.length,
      i
    for (i = 0; i < _len; i++) {
      var _eTD = _tds[i]
      if (_eTD && _eTD.tagName == 'TD') {
        $(_eTD).css('height:' + _tdH + 'px;width:' + _tdW + 'px;')
      }
    }
  }
...
$.UI.Tab = function (j) {
  ...
  function layout () {
    btnSet = new $.UI.ButtonSet({
      ...
      onClose: function (obj) {
        me.closeTab(obj.Name)
      }
    })
    toolSet = new $.UI.ButtonSet({
      p: eHead,
      itemAlign: 'right',
      items: args.toolBarAry,
      onClick: function (obj) {
        obj.Tab = me
        args.onToolBarClick(obj)
      }
  ...
  me.getLenth = function () {
    return me.aItems.length
  }
  ...
  function clickTab (v) {
    ...
    if (_item) {
      ...
      if (me.selItem) {
        if (me.selItem.Name == _name) {}
        me.selItem.Body.hide()
      }
      if (args.loadMode == 'click') {
        var _eBody = _item.Body,
          _cStr = _item.Args.content,
          _js = _item.Args.js,
          _url = _item.Args.url
        if (_cStr) {
          _eBody.h(_cStr)
  ...
  me.getTabBtn = function (v) {
    return btnSet.getItem(v)
  }
  me.setSelTab = function (v) {
    btnSet.fireClick(v, null, true)
    return me
  }
  me.clear = function () {
    for (var i = 0, _len = me.aItem.length; i < _len; i++) {
  ...
  me.addTabItem = function (j, ifShow) {
    ...
    var _btn = btnSet.addItem(j, true),
      _body = eBody.adElm('', 'div').hide()
    var _item = {
      Button: _btn,
      Body: _body,
      Args: j,
  ...
  me.setTabText = function (k, v) {
    btnSet.setText(k, v)
    return me
  }
  me.setTabIcon = function (k, v) {
    btnSet.setIcon(k, v)
    return me
  }
...
$.UI.Slider = function (j) {
  ...
  var aIndex = [],
    IntervalID, _pW, _pH, _ifInit = true,
    orig_pBody, ifArrow
  var eNext, ePre
  me.items = []
  me.currItem = null
  ...
  function setDef (j) {
    ...
    if (!pHead) {
      var root_BaseDiv = new $.UI.BaseDiv({
        ...
        skin: {
          ...
          foot: {
            cn: 'oh FFsn slider_nav',
            css: ''
          }
        }
      })
      pBody = root_BaseDiv.body
      pHead = root_BaseDiv.foot
      pBody.ac('oh').dc('oa')
    }
    ...
    _pW = pBody.csn('width')
    _pH = pBody.csn('height')
    pBody = pBody.adElm('', 'div').cn('pa wp hp oh')
    if (dir != 'none' && dir != 'fade') {
      pBody = pBody.adElm('', 'div').cn('pa oh')
    }
  ...
  function bindEvent () {
    ...
    ePre.evt('click', function (e) {
      var e = $.e.fix(e)
      me.pre()
    })
    eNext.evt('click', function (e) {
      var e = $.e.fix(e)
      me.next()
    })
  }
  me.addItem = function (j) {
    ...
    var _name = _j.name || ('slider_' + _currC),
      _title = _j.title,
      _link = _j.link || 'javascript:void(0);'
    var _cn = _j.cn || 'fr tac fs10 mr5 mt5 w15 h15 lh15 r15',
      _css = _j.css || '',
      _press = _j.press || false
    if (_press) {
      _cn += ' ' + currCn
    } else {
      _bodyCn += ' dn'
    ...
    if (_text && pText.className.ec('dn')) {
      pText.dc('dn')
    }
    var _items = {
      body: b,
  ...
  me.start = function (dly) {
    ...
    IntervalID = setInterval(function () {
      var _index = -1
      if (me.currItem) {
        _index = me.currItem.index
      }
      _index++
      if (_index == aIndex.length) {
        _index = 0
      }
  ...
  me.setSelItem = function (v, dirction, t, cbFn) {
    var _item = me.items[v],
      currI = me.currItem,
      _d = dirction || dir
    if (!_item || _item == currI) {
      return
    }
    var b = _item.body,
      h = _item.head,
    ...
    switch (_d) {
      ...
      case 'bottom':
        ...
        pBody.ease(['top'], [changeVal], 600, 'easeOutQuart', {
          e: function () {}
        })
        if (currI) {
          var currH = currI.head,
      ...
      case 'right':
        ...
        if (_ifInit) {
          ...
          pBody.ease(['left'], [changeVal], 600, 'easeOutQuart', {
            e: function () {}
          })
        } else {
          var _lc = $(pBody.lastChild),
            _fc = $(pBody.firstChild)
          _lc.moveB(_fc)
          pBody.ease(['left'], [0], 600, 'easeOutQuart', {
            e: function () {}
          })
        }
        if (_idx == _fIdx) {
...
$.UI.ScrollBar = function (j) {
  ...
  function bindEvent () {
    ...
    hBar.evt('mousedown', function (e) {
      ...
      _e.onDrag = function (diff, b) {
        owner.css('margin-left:-' + (diff / hRate * 100) + 'px;')
        args.onDrag()
      }
      _e.onDragEnd = function () {
        args.onDragEnd()
      }
      $.drag.start(e, _e)
      e.stop()
  ...
  me.addElm = function (obj) {
    var _obj = obj || {},
      _type = _obj.type,
      _onSuc = _obj.onSuccess || _fn
    _obj.p = owner
    _obj.onSuccess = function () {
      me.initBar()
      _onSuc({
        ScrollBar: me,
        Args: obj
  ...
  me.initBar = function () {
    ...
    var _pW = p.csn('width'),
      _pH = p.csn('height'),
      _pW1 = owner.csn('width'),
      _pH1 = owner.csn('height')
...
$.UI.Accordion = function (j) {
  ...
  var args = {
      ...
      items: [],
      onItemExpand: _fn,
      onItemToolBarClick: _fn
    },
    iArgs
  var owner, count, ifInit = true,
    curr, sumHeight = 0
  me.items = {}
  me.aItem = []
  me.selItem = null
  ...
  function layout () {
    ...
    me.setSelItem(0)
    $(window).evt('resize', function () {
      var _val = args.p.csn('height') - sumHeight
      for (var i = 0, _len = me.aItem.length; i < _len; i++) {
        me.aItem[i].owner.css('height:' + _val + 'px;')
      }
    })
  }
  ...
  function onItemExpand (obj) {
    if (args.ifFixedHeight) {
      ...
      if (me.selItem && me.selItem != obj) {
        me.selItem.setFold(false, null, false)
      }
      me.selItem = obj
    }
    args.onItemExpand({
  ...
  me.addItem = function (j) {
    var _iArgs = $.Util.initArgs(j, iArgs),
      _name = _iArgs.name
    _iArgs.ifUnfold = false
    _iArgs.titleHeight = args.titleHeight
    sumHeight += (args.titleHeight || 0) + 2
    var _item = new $.UI.AttrPanel(_iArgs),
      _idx = count.getN()
    _item.evt('onExpand', function (obj) {
      onItemExpand(obj.AttrPanel)
    }).evt('onToolBarClick', onItemToolBarClick)
    me.items[_name] = me.items[_idx] = _item
    me.aItem.push(_item)
    return _item
  }
  ...
  me.resize = function () {
    var _allH = owner.csn('height'),
      _itemH = _allH - (me.aItem.length * (args.titleHeight + 1.5))
    for (var i = 0, _iLen = me.aItem.length; i < _iLen; i++) {
      me.aItem[i].owner.css('height:' + _itemH + 'px;')
    }
    return me
  }
  me.getArgs = function () {
...
$.UI.BarCode = function (j) {
  ...
  me.setValue = function (value) {
    ...
    var _obj = {
      code: $.Util.code.code128.getDigit(value)
    }
    eStr.h(value)
...
$.UI.FileBrowser = function (j) {
  ...
  function bindEvent () {
    eBody.evt('click', function (e) {
      ...
      if (_obj.tag == 'flag' && _dir) {
        ...
        switch (_cn) {
          case 'flag-close':
            loadDirFiles(_dir, _eLi, function (len) {
              if (len) {
                _e.cn('flag-open')
              } else {
                _e.cn('')
          ...
          case 'flag-open':
            disposeChild(_eLi)
            _e.cn('flag-close')
            _eIcon.cn('icon-close')
        }
        args.onExpand(_obj)
      } else {
        if (_eLi.className == 'dir') {
          args.onFoldClick()
        } else {
  ...
  function disposeChild (eStart) {
    if (eStart) {
      removeChild(eStart.ns(), eStart.attr('_depth'))
    }
  }

  function removeChild (child, depth) {
    if (child && child.attr('_depth') != depth) {
      removeChild(child.ns(), depth)
      child.r()
      child = null
    }
  }
...
$.UI.FileUploader = function (j) {
  ...
  function layout () {
    ...
    var _eBarP = eHead.chn(1).fc(),
      _spAry = args.specialFiles,
      _spLen = _spAry.length
    toolBar = new $.UI.ButtonSet({
      ...
      items: [{
        name: 'add',
        text: '添加',
        icon: 'icon-glyph-plus-sign'
      },
      {
        name: 'start',
        text: '开始',
        state: 'stop',
        icon: 'icon-glyph-circle-arrow-up'
      }
      ]
    })
    proBar = new $.UI.ProgressBar({
      p: _eBarP,
      css: 'margin-top:10px;'
    })
    if (_spLen) {
      ...
      for (var i = 0; i < _spLen; i++) {
        var _fAry = me.addSpecialFile(_spAry[i]),
          _name = _fAry[0],
          _fName = "'" + _name + "'"
      ...
      getSpeciFilesState(_fileAry, function (d) {
        var _data = d.data[0]
        if (_data) {
          var _rAry = eval(_data),
            _rLen = _rAry.length
          for (var _r = 0; _r < _rLen; _r++) {
            var _rn = _rAry[_r]['origName']
            _checkObj[_rn] = _rAry[_r]['id']
          }
        }
        for (var _idx in _eCheckObj) {
          var _eImg = _eCheckObj[_idx],
            _val = _checkObj[_idx] || 0
  ...
  function setUploadState (filename) {
    currIdx++
    var _len = filename.length,
      _bv = $.m.p(currIdx / fileCount * 100, 2)
    if (_len > 40) {
  ...
  function getUploadUrl () {
    var _url = $.global.getImgPath() + 'Module/SYS_CM_FILES.aspx?',
      _api = args.uploadApi || ('action=uploadFile&mId=' + args.mId + '&catelogId=' + args.catelogId + '&catelog=' + args.catelog)
    return _url + _api
  }
  ...
  function checkFileType (type, extFile) {
    ...
    if (!extFile) {
      extFile = args.denyFileFormat
    }
    var _strReg = '',
      _type = $.getType(extFile)
    if (_type == 'string') {
      _strReg = extFile
    };
    if (_type == 'array') {
      _strReg = extFile.join('|')
    }
  ...
  function uploadCallback (isTimeout) {
    ...
    if (currObj) {
      ...
      if (status == 'success') {
        ...
        if (currObj['eState']) {
          currObj['eState'].dc('error_16').dc('check-not-exist').ac('ok_16').attr('title', '已经上传过了')
        }
        currSuccFileAry.push(_fObj)
        succFileAry.push(_fObj)
        currIds.push(_fObj.id)
        args.onSuccess({
          FileUploader: me,
          File: _fObj,
          State: currObj
  ...
  function onUploadFinish () {
    eFileName.h('')
    fileCount = currIdx = 0
    for (var i = 0; i < errorFnAry.length; i++) {
      queue.push(errorFnAry[i])
    }
    var _obj = {
      responseText: xml,
      status: status,
  ...
  me.submit = function () {
    if (_ifInit) {
      if (denyFileAry.length || !fileCount) {
        return false
      }
      itemCount = queue.len()
      proBar.setVal(0)
  ...
  me.addSpecialFile = function (j) {
    ...
    var _eForm = _eItem.fc(),
      _eDetail = _eForm.fc().h(htmlTemp.spDetail.format('icon-filetype-' + _ext, _file)),
      _eOper = _eDetail.ns()
    var _eInput = _eOper.fc().chn(1).alpha(0),
      _eState = _eDetail.chn(1).chn(2).chn(1)
    var _obj = {
      f: function () {
        if (_eDetail.chn(1)) {
          if (!_eInput.value) {
          ...
          var _eD = _eDetail.chn(1),
            _strFN = _eD.fc().h(),
            _fid = +_eState.attr('fid')
          _eOper.fc().ac('dn')
          $(_eOper.lastChild).dc('dn')
          currObj = {
            eImg: $(_eOper.lastChild),
            eState: _eState,
            fileName: _strFN,
            ifSpecial: true,
            fid: _fid
    ...
    _eInput.evt('change', function (e) {
      var e = $.e.fix(e),
        _e = e.t
      proBar.setVal(0, 1)
      var _obj = urlToObj(_e),
        _fType = _obj['fileType'].toLow()
      if (checkFileType(_fType, _ext)) {
        ...
        if (_ext == 'default') {
          _eDetail.fc().fc().className = 'icon-filetype-' + _fType
        }
  ...
  function urlToObj (_e) {
    ...
    if (_e.files) {
      ...
      _fullName = _file.name
      _ary = _fullName.split('.')
      _name = _ary[0]
      _fileType = _ary.pop()
      _size = _file.size
    } else {
...
$.UI.FlowChart = function (j) {
  ...
  function bindEvent () {
    $D.evt('keydown', function (e) {
      ...
      if (downFlag && downMc) {
        ...
        if (!lineMask) {
          lineMask = owner.adElm('', 'div').cn('pa bc_10 wp hp z3').alpha(0)
        }
    ...
    owner.evt('click', function (e) {
      ...
      if (_e == owner) {
        if (wf_me.selElm) {
          wf_me.selElm.dc('node-sel').ac('node-normal')
        }
        wf_me.selElm = null
        wf_me.selPoint = null
        args.onOwnerClick({
        ...
        if (wf_me.selElm && (wf_me.selElm.className.indexOf('node-normal') == -1)) {
          wf_me.selElm.dc('node-sel').ac('node-normal')
        }
        var _node = wf_me.hashPoint[_e.id],
          _ecn = _e.className
        var _cbReturn = {
          FlowChart: wf_me,
          _e: _e,
        ...
        if (_ecn == 'line') {
          wf_me.selPoint = null
          _cbReturn.Line = _node
          args.onLineClick(_cbReturn)
        } else {
          wf_me.selElm = _e
          wf_me.selPoint = _node
          if (_ecn.indexOf('node-sel') == -1) {
            _e.dc('node-normal').ac('node-sel')
          }
          _cbReturn.Point = _node
          args.onPointClick(_cbReturn)
        }
      }
    }).evt('mouseup', function (e) {
      var e = $.e.fix(e),
      ...
      if (downKey == 17 && downFlag == true) {
        ...
        if (wf_me.selElm) {
          var _selID = wf_me.selElm.id,
            _selPoint = wf_me.hashPoint[_selID]
          if (overElm) { // 连接一个已经存在的节点
            var _overPoint = wf_me.hashPoint[overElm.id]
            if (_selPoint.next.indexOf(overElm.id) >= 0) {
              return
            }
            if (args.onLinkEnd({
              FlowChart: wf_me,
            ...
            var _de = $D.documentElement,
              _oPos = owner.pos()
            var sX = _selPoint.x,
              sY = _selPoint.y
            var eX = _de.scrollLeft + e.clientX - _oPos.x,
              eY = _de.scrollTop + e.clientY - _oPos.y
            if (args.ifDragCreate && args.onLinkEnd({
              FlowChart: wf_me,
              Start: _selPoint,
              SX: sX,
              SY: sY,
  ...
  function joinNode (obj) {
    var _j = obj || {},
      _pre = _j.pre,
      _next = _j.next
    var _pNode = wf_me.hashPoint[_pre],
      _nNode = wf_me.hashPoint[_next]
  ...
  function createLine (j) {
    ...
    var x1 = j.x1 || 0,
      ...
      _next = j.next || '',
      _pre = j.pre || ''
    var lineObj = j.lineObj || new $.draw({
      p: owner
    })
    var _txt = j.txt || '',
      _id = j.id
  ...
  function createPoint (j) {
    ...
    point.setExt = function (key, value) {
      if (!key) {
        return
      }
      point.ext[key] = value
    ...
    point.getExt = function (key) {
      return point.ext[key]
    }

    function resetLines (_x, _y) {
      var _preAry = point.pre.split(' '),
        _p_len = _preAry.length
      var _nextAry = point.next.split(' '),
        _n_len = _nextAry.length
      // 父亲连线
      for (var i = 0; i < _p_len; i++) {
        var _pre = _preAry[i],
          _key = _pre + '#' + _id,
          _pNode = wf_me.hashPoint[_pre]
        if (!wf_me.hashPoint[_key]) {
          continue
        }
      ...
      for (var i = 0; i < _n_len; i++) {
        var _next = _nextAry[i],
          _key = _id + '#' + _next,
          _pNext = wf_me.hashPoint[_next]
        if (!wf_me.hashPoint[_key]) {
          continue
        }
  ...
  function deletePoint (key) {
    var _point = wf_me.hashPoint[key],
      _type
    if (!_point) {
      return
    }
    ...
    switch (_type) {
      case 'point':
        var aPre = _point.pre.split(' '),
          _p_len = aPre.length
        var aNext = _point.next.split(' '),
          _n_len = aNext.length
        for (var i = 0; i < _p_len; i++) {
          var _pre = aPre[i],
            _prePoint = wf_me.hashPoint[_pre]
          if (_prePoint) {
        ...
        for (var i = 0; i < _n_len; i++) {
          var _next = aNext[i],
            _nextPoint = wf_me.hashPoint[_next]
          if (_nextPoint) {
  ...
  wf_me.firePointClick = function (id, fn) {
    var _point = wf_me.hashPoint[id],
      _f = fn || args.onPointClick
    if (!_point) {
      _point = wf_me.firstPoint
    }
    if (_point) {
      var elm = _point.elm
      if (wf_me.selElm) {
  ...
  wf_me.getCurrIdx = function () {
    return nCount.getN()
  }
  wf_me.addNode = function (j, ifReturn) { // 创建一个节点
    var point
    if (j.type == 'point') {
      point = new createPoint(j)
    } else {
      point = new createLine(j)
    }
    if (ifReturn) {
      return point
    }
  ...
  wf_me.getNode = function (key) {
    if (typeof key === 'string') {
      return wf_me.hashPoint[key]
    } else {
      return key
    }
  }
  wf_me.init = function (j) {
    setDefault(j)
    layout()
    bindEvent()
    return wf_me
...
$.UI.Chat = function (j) {
  ...
  var socket, _dialogs = {},
    _userHash = {},
    _currUserId = $.ck.get('SESSIONID'),
    _socketId, _socketHash = {}
  var owner, args = {
    p: $DB,
    cn: '',
    css: '',
    socketServer: 'http://127.0.0.1:8081'
  }
  ...
  function bindEvent () {
    owner.evt('click', function (e) {
      ...
      if (_e.tagName == 'A' && _e.className == 'talk') {
        if (_e.pn().className.ec('gray')) {
          return
        }
        var _uid = +_e.attr('uid')
        if (!_dialogs[_uid]) {
          _dialogs[_uid] = new $.UI.ChatDialog({
            socket: socket,
            userName: _e.attr('uName'),
            userId: _uid
          })
        }
        _dialogs[_uid].show()
    ...
    socket.on('pushOnLineUsers', function (data) {
      ...
      var _hash = data.socketHash,
        _sId = socket.socket.sessionid
      for (var k in _hash) {
        var _userObj = _userHash[+_hash[k].loginId]
        if (k != _sId && _userObj && _userObj.eLi) {
          _userObj.eLi.dc('gray')
        }
      }
    })
    socket.on('notice', function (data) {
      var _cId = +_currUserId,
        _fId = +data.from,
        _tId = +data.to
      data.fromValue = _userHash[_fId]
      data.toValue = _userHash[_tId]
      if (_cId == _fId) {
        data.ifSelf = true
      } else {
        data.ifSelf = false
      ...
      if (_cId == _fId || _cId == _tId) {
        ...
        if (_dialog) {
          ...
          _dialog = _dialogs[_fId] = new $.UI.ChatDialog({
            socket: socket,
            userName: data.fromValue.uid,
            userId: data.from
          })
        }
        _dialog.addMsg(data)
...
$.UI.ChatDialog = function (j) {
  var me = this,
    _fn = function () {}
  var owner, dialog, eInput, eMsg, socket, args = {
    p: $DB,
    socket: null,
    userName: '',
    userId: 0
  ...
  function layout () {
    ...
    var contentLayout = new $.UI.Layout({
      p: mainLayout.eHead,
      min: 150,
      max: 250,
      ifCover: false,
    ...
    var baseDiv = new $.UI.Tips({
      p: contentLayout.eFoot,
      cn: 'b0',
      head_h: 0,
      foot_h: 38
    })
    ...
    loadUser(args.userId, mainLayout.eFoot.ac('chat-user-info'))
  }
  ...
  function addMsgItem (obj) {
    var _dir = 'left',
      _bc = '#E2E2E2',
      _bbc = '#D9F0F7',
      _arrowH = '<div class="pa w16 h16 fl" style="margin-left:-16px;padding:0px;margin-top:5px;"></div>'
    if (obj.ifSelf) {
      _dir = 'right'
      _arrowH = '<div class="pa w16 h16 fr" style="padding:0px;margin-top:5px;right:10px;"></div>'
      _bbc = '#FFFFCA'
    }
    ...
    new $.UI.Arrow({
      ...
      comMode: 'border',
      borderColor: _bc,
      backgroundColor: _bbc,
      cn: 'cp',
      dir: _dir
    })
    eMsg.scrollTop = eMsg.offsetHeight - eMsg.clientHeight // Chrome
  }
  me.addMsg = function (obj) {
    addMsgItem({
      ifSelf: false,
      name: obj.toValue.uid,
      msg: obj.msg
    })
  }
  me.getArgs = function () {
...
$.UI.View = function (j) {
  ...
  me.loadView = function (_args) {
    if ($.global.ifCheckCookie && !$.ck.get('SESSIONID')) {
      MTips.show('您已经很长时间未操作了哦，请重新登录！', 'warn')
      window.location.href = 'index.html'
      return
    }
    var _args = _args || {},
      _url = _args.url || '',
      _f = args.onLoad || args.onLoad,
      _comArgs = _args || _args.args
    if (!_url) {
      return
    }
    if (!_comArgs) {
    ...
    $.Util.loadJS(_url, function () {
      args.p.h('')
      var _ns = urlToNS(_url)
      _comArgs.p = args.p
      if (!_ns) {
        me.loadView({
          url: 'View/error/Error404.js',
          href: _url
        })
        return
      };
      view = new _ns(_comArgs)
      _f(view, me)
    })
  }
  init(j)
...
$.UI.ViewClass = function () {
  this.owner = null
  this.args = {}
  this.self = null
}
$.UI.ViewClass.prototype = {
  setArgsObj: function (v) {
    this.args = v
  },
  setOwner: function (v) {
    this.owner = v
